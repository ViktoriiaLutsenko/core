...
public class WebDriverFactory{
    ...
    private final Supplier<RemoteWebDriver> REMOTE_CHROME = () -> {
        try {
            return new RemoteWebDriver(URI.create(System.getProperty("selenium.grid.url")).toURL(), new ChromeOptions());
    ...
    private final Supplier<RemoteWebDriver> REMOTE_FIREFOX = () -> {
        try {
            return new RemoteWebDriver(URI.create(System.getProperty("selenium.grid.url")).toURL(), new FirefoxOptions());
        } catch (MalformedURLException e) {
            throw new RuntimeException(e);
        }
    };
    ...
    public WebDriver getDriver() {
        String runMode = System.getProperty("selenium.grid.enabled");
